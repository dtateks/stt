...
fn bridge_payload_keys_match_rust_command_signatures() {
    ...
    assert!(
        bridge_js.contains("invoke(\"consume_pending_mic_toggle\")"),
        "consume_pending_mic_toggle bridge call must be present"
    );
    assert!(
        bridge_js
            .contains("invoke(\"fit_main_window_to_content\", { content_height: contentHeight })"),
        "fit_main_window_to_content payload must use snake_case content_height"
    );
    assert!(
